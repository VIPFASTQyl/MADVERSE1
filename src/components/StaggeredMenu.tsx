...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const animateIcon = useCallback((opening: boolean) => {
    // Icon animation handled by Tailwind CSS rotate class
  }, []);
    ...
    <div className={`sm-scope z-40 ${isFixed ? 'fixed top-0 left-0 w-screen h-screen overflow-hidden' : 'w-full h-full'}`}>
      ...
      >
        <div ref={preLayersRef} className="sm-prelayers absolute top-0 right-0 bottom-0 pointer-events-none z-[5]" aria-hidden="true">
          {(() => {
            const raw = colors && colors.length ? colors.slice(0, 4) : ['#000', '#1a1a1a'];
            let arr = [...raw];
            if (arr.length >= 3) {
        ...
        <header className="staggered-menu-header absolute top-0 left-0 w-full flex items-center justify-between p-[2em] bg-transparent pointer-events-none z-20" aria-label="Main navigation header">
          ...
          <div className="flex items-center gap-2">
            <button
              ref={toggleBtnRef}
              className={`sm-toggle relative inline-flex items-center gap-0 bg-transparent border-0 cursor-pointer font-medium leading-none overflow-visible pointer-events-auto ${
            ...
            >
              <span ref={textWrapRef} className="sm-toggle-textWrap relative inline-block h-[1em] overflow-hidden whitespace-nowrap w-[var(--sm-toggle-width,auto)] min-w-[var(--sm-toggle-width,auto)]" aria-hidden="true">
                <span ref={textInnerRef} className={`sm-toggle-textInner flex flex-col leading-none ${open ? 'text-black' : ''}`}>
                  {textLines.map((l, i) => (
              ...
              <Plus size={20} className={`ml-1 transition-transform ${open ? 'rotate-45 text-black' : ''}`} />
            </button>
          </div>
        </header>
...
.sm-scope .sm-panel-itemWrap { position: relative; overflow: hidden; line-height: 1; border-bottom: 1px solid rgba(255, 255, 255, 0.6); }
