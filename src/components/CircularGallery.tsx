...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = '#545050', font = 'bold 30px Figtree' }: TitleProps) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
class App {
  ...
  handleCanvasClick(e: MouseEvent) {
    ...
    if (!this.renderer || !this.renderer.gl || !this.viewport || !this.medias) return;
    const canvas = this.renderer.gl.canvas as HTMLCanvasElement;
    const rect = canvas.getBoundingClientRect();
    const clientX = e.clientX - rect.left;
    const ndcX = (clientX / rect.width) * 2 - 1; // -1..1
    const worldX = ndcX * (this.viewport.width / 2);

    // Find nearest media
    let nearest: Media | null = null;
    ...
    if (nearest && this.onItemClick) {
      const category = String(nearest.text || '').toLowerCase();
  ...
  createRenderer() {
    ...
    // Make canvas clickable for selecting a media item
    const canvas = this.renderer.gl.canvas as HTMLCanvasElement;
    canvas.style.touchAction = 'manipulation';
    canvas.addEventListener('click', (e) => this.handleCanvasClick(e as MouseEvent));
  }
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    if (Math.abs(this.clickStart - x) > 5) {
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    // wire navigation when a media is clicked
    app.onItemClick = (category: string) => {
      if (!category) return;
